...
设计模式是为**解决某些普遍存在的问题**提出的**方案与指导原则**。它们不是类，程序包或库文件，没法直接放到您的应用程序里然后等待神奇的事情发生。确切地说，它们是一种指导原则，旨在告诉您可以选用某种方法，来解决一类问题。
...
> 为了修建一栋房子，您需要一扇门。您可以穿上木匠的工作服，拿上木头、胶水、钉子等必要的工具，在您的房子里亲自制作一扇门；或者，您只需要打个电话给工厂，让他们把制造好的门送到您身边 —— 这样您不需要了解任何关于制作门的知识，也不必处理制作门可能带来的麻烦。
...
> 以招聘经理为例。一位面试官不可能面试所有的职位。根据职位空缺情况，招聘经理必须制定面试的步骤，并将面试任务委派给不同的面试官。
...
> 让我们拓展简单工厂模式里关于门的例子。根据您的需要，您可能会从木工房买到木门，从铁工房买到铁门，或是从对应的工房买到塑料门。现在，您可能需要有着不同专业能力的师傅来安装这些门，比如让木匠安装木门，让焊工安装铁门。如前所述，现在门与安装师傅之间存在一种依赖关系，木门需要木匠，铁门需要焊工。
...
现在我们有了抽象工厂，它允许我们创建一系列相关联的对象，即木工房能够制造木门并提供安装木门的师傅，而铁工房能够制造铁门并提供安装铁门的师傅。
...
正如您看到的，木工房已经封装了 `carpenter`（木匠）和 `wooden door`（木门），而铁工房已经封装了 `iron door`（铁门）和 `welder`（焊工）。因此，抽象工厂确保了对于每一个制造出来的门，我们都能得到正确的安装师傅。

**什么时候使用？**

当对象之间存在相互依赖的关系，或对象涉及的创建逻辑并不那么简单时。
...
> 想象您在哈帝斯汉堡店里，点了一份“大哈迪汉堡”。准备好后，店员就把汉堡递给您（_毫无疑问_ 这是一个简单工厂的例子）。但是在某些情况下，制作汉堡可能会包括额外的步骤逻辑。举个例子，您想要一份定制的汉堡，有很多定制选项：您要什么面包片？喜欢哪款酱汁？想吃哪种奶酪？诸如此类。在这种情况下，就需要用到生成器模式了。
...
> 求职者就是很好的例子，他们会订阅一些职位发布网站，当存在匹配的工作机会时网站会通知他们。
...
实现求职者的例子。首先，我们定义了求职者类，当有新的职位发布时，他们将得到通知
